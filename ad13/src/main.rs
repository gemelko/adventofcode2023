                ...
                while cur_line >= 0 {
                    ...
                    for next_line in cur_line+1..group_count as i32 {
                        if cur_line*2+1-next_line >= 0 {
                            let difval = count_diffs(&this_group[next_line as usize], &this_group[(2*cur_line+1-next_line) as usize]);
                            diffs += difval as i32;
                            if diffs>1 {
                                matching = false;
                                break;
                    ...
                    if matching && diffs==1 && cur_line>=0 {
                ...
                for cpos in (1..this_len).rev() {
                    ...
                    for this_line in this_group.clone() {
                        if matching1 && len1>0 {
                            ...
                            let difval = count_diffs(&left, &right);
                            diffs += difval as i32;
                            if diffs>1 {
                                matching1 = false;
                            }
